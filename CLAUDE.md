...
The gate's `pnpm/action-setup` version and `actions/setup-node`'s `node-version` must move together: pnpm 11.17.0 uses the built-in `node:sqlite` module at runtime and requires Node >= 22.13, so bumping one without the other reintroduces `ERR_UNKNOWN_BUILTIN_MODULE`. The remote script bootstraps `$SSH_DIR` with `git clone` if it isn't already a git repo (first-ever deploy, or a wiped directory) — it can't recover from a directory that exists, is non-empty, and isn't a git repo.
